...
mod tests {
    use super::*;
    use crate::model::{AlertLine, AlertMessageKey, AlertSurface, StrikePlan};
    use anyhow::Result;
    ...
    impl AlertMessageRepository for BlockingPruneRepository {
        async fn record_message(&self, _key: &AlertMessageKey, _lines: &[AlertLine]) -> Result<()> {
        ...
        async fn commit_strikes(&self, _key: &AlertMessageKey, _lines: &[u32]) -> Result<()> {
            unimplemented!("not used by these tests")
        }

        async fn forget_message(&self, _key: &AlertMessageKey) -> Result<()> {
            unimplemented!("not used by these tests")
        }
